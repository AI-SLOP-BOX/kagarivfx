...
>
> **あなたに担当してほしいネクストタスク**:
> 1. **複数レイヤーの一括選択・グループ化機能 (`timeline.rs`)**: タイムラインで `Shift` クリックによる複数レイヤー選択と、選択グループのトランスフォーム一括適用。
> 2. **キーフレームスナップ機能**: タイムラインスクラブ時のキーフレーム・マーカーへの磁石スナップ操作。
> 3. **ドッキングパネルシステム (`egui_dock`)**: `egui_dock` クレートを導入し、ユーザーがパネルを自由にドラッグ&ドロップで再配置できるようにする。
```
