...
pub fn apply_sphere_env_map(pixels: &mut [u8], width: u32, height: u32, env_map: &[u8], env_width: u32, env_height: u32) {
        ...
        for x in 0..width {
            ...
            if r2 <= 1.0 {
                ...
                if env_idx + 3 < env_map.len() {
                    for c in 0..4 {
                        pixels[dst_idx + c] = ((temp[dst_idx + c] as u16 + env_map[env_idx + c] as u16) / 2) as u8;
                    }
                }
            }
        }
...
pub fn apply_z_depth_fog(pixels: &mut [u8], depth_map: &[u8], fog_color: [u8; 3], fog_near: f32, fog_far: f32) {
    for i in (0..pixels.len()).step_by(4) {
        let depth = if i < depth_map.len() { depth_map[i] as f32 / 255.0 } else { 1.0 };

        let fog_factor = ((depth - fog_near) / (fog_far - fog_near).max(0.001)).clamp(0.0, 1.0);

        for c in 0..3 {
            pixels[i + c] = (pixels[i + c] as f32 * (1.0 - fog_factor) + fog_color[c] as f32 * fog_factor).clamp(0.0, 255.0) as u8;
        }
    }
}

// 3. God Rays (Volumetric Radial Light Scattering)
pub fn apply_god_rays(pixels: &mut [u8], width: u32, height: u32, sun_pos: [f32; 2], num_samples: u32, decay: f32, weight: f32) {
...
pub fn apply_ssao_approx(pixels: &mut [u8], width: u32, height: u32, depth_map: &[u8], radius: u32, strength: f32) {
    if radius == 0 { return; }
    ...
    for y in 0..height as i32 {
        for x in 0..width as i32 {
            let idx = (y as usize * width as usize + x as usize) * 4;
            let center_depth = if idx < temp_depth.len() { temp_depth[idx] as f32 } else { 128.0 };
            ...
            for ky in -r..=r {
                ...
                for kx in -r..=r {
                    ...
                    let neighbor_depth = if n_idx < temp_depth.len() { temp_depth[n_idx] as f32 } else { 128.0 };
...
pub fn apply_gobo_projection(pixels: &mut [u8], width: u32, height: u32, cookie: &[u8], light_pos: [f32; 2], falloff: f32) {
    for y in 0..height {
        for x in 0..width {
            let idx = (y as usize * width as usize + x as usize) * 4;
