...
impl GpuComputeContext {
    ...
    pub fn new() -> Option<Self> {
        ...
        let label = format!("{} ({})", info.name.trim(), info.backend.to_str());
        // Leak the instance so its Metal backend destructor does not run at
        // process exit.  The Device + Queue hold their own Arc references to
        // the wgpu-core context, so the Instance is no longer needed.
    ...
    fn gaussian_blur_inner(
        ...
        bufs.staging_out.mark_mapped();
        inner.device.poll(wgpu::Maintain::Wait);
        let map_ok = rx.recv().ok().and_then(|r| r.ok()).is_some();
        if !map_ok {
            bufs.staging_out.unmap_tracking();
            return None;
        }
        {
    ...
    pub fn gpu_color_tint(
        ...
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                mode: 12,
                angle: 0.0,
                brightness: tint_rgb[0],
                contrast: tint_rgb[1],
                saturation: tint_rgb[2],
                hue_shift: intensity,
                param_f3: 0.0,
                param_f4: 0.0,
                param_f5: 0.0,
    ...
    pub fn gpu_levels(
        ...
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                mode: 15,
                angle: 0.0,
                brightness: in_black / 255.0,
                contrast: in_white / 255.0,
                saturation: gamma,
                hue_shift: out_black / 255.0,
                param_f3: out_white / 255.0,
                param_f4: 0.0,
                param_f5: 0.0,
                param_f6: 0.0,
                param_f7: 0.0,
    ...
    pub fn gpu_offset(
        ...
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                saturation: 1.0,
                hue_shift: 0.0,
                param_f3: shift_x as f32,
                param_f4: shift_y as f32,
                param_f5: 0.0,
                param_f6: 0.0,
                param_f7: 0.0,
                param_f8: 0.0,
    ...
    pub fn gpu_chromatic_aberration(&self, pixels: &mut [u8], w: u32, h: u32, amount: f32) -> bool {
        self.dispatch_fx(
            pixels,
            w,
            h,
    ...
    pub fn gpu_minimax(
        ...
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                param_f3: 0.0,
                param_f4: 0.0,
                param_f5: if maximize { 1.0 } else { 0.0 },
                param_f6: 0.0,
                param_f7: 0.0,
                param_f8: 0.0,
                _pad: 0.0,
    ...
    pub fn gpu_alpha_from_luminance(&self, p: &mut [u8], w: u32, h: u32, invert: bool) -> bool {
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                mode: 34,
                angle: 0.0,
                brightness: if invert { 1.0 } else { 0.0 },
                contrast: 1.0,
                saturation: 1.0,
                hue_shift: 0.0,
                param_f3: 0.0,
    ...
    pub fn gpu_luma_key_range(
        ...
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                brightness: low,
                contrast: high,
                saturation: if invert { 1.0 } else { 0.0 },
                hue_shift: 0.0,
                param_f3: 0.0,
                param_f4: 0.0,
                param_f5: 0.0,
    ...
    pub fn gpu_tritone(
        ...
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                param_f3: mid[1],
                param_f4: mid[2],
                param_f5: highlight[0],
                param_f6: highlight[1],
                param_f7: highlight[2],
                param_f8: 0.0,
                _pad: 0.0,
            },
            "gpu_tri",
    ...
    pub fn gpu_polar_coords(&self, p: &mut [u8], w: u32, h: u32, to_polar: bool) -> bool {
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                mode: 41,
                angle: 0.0,
                brightness: if to_polar { 1.0 } else { 0.0 },
                contrast: 1.0,
                saturation: 1.0,
                hue_shift: 0.0,
                param_f3: 0.0,
    ...
    pub fn gpu_optics_comp(&self, p: &mut [u8], w: u32, h: u32, fov: f32, reverse: bool) -> bool {
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                angle: 0.0,
                brightness: fov,
                contrast: if reverse { 1.0 } else { 0.0 },
                saturation: 1.0,
                hue_shift: 0.0,
                param_f3: 0.0,
                param_f4: 0.0,
    ...
    pub fn gpu_directional_sharpen(
        &self,
        p: &mut [u8],
        w: u32,
        h: u32,
    ...
    pub fn gpu_chroma_key(
        ...
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                mode: 56,
                angle: 0.0,
                brightness: key_rgb[0],
                contrast: key_rgb[1],
                saturation: key_rgb[2],
                hue_shift: gain,
                param_f3: softness,
                param_f4: 0.0,
                param_f5: 0.0,
    ...
    pub fn gpu_glitch_displacement(
        &self,
        p: &mut [u8],
        w: u32,
        h: u32,
    ...
    pub fn gpu_reflection_map(
        ...
        self.dispatch_fx(
            ...
            ParamsUniform {
                ...
                mode: 62,
                angle: 0.0,
                brightness: if reflect_y { 1.0 } else { 0.0 },
                contrast: fade,
                saturation: 1.0,
                hue_shift: 0.0,
                param_f3: 0.0,
    ...
    fn alloc_buf_set(&self, dev: &wgpu::Device, buf_len: u64) -> BufSet {
        ...
        BufSet {
            ...
            staging_out: StagingBuffer::new(dev.create_buffer(&wgpu::BufferDescriptor {
...
#[cfg(not(test))]
static GLOBAL: OnceLock<Option<&'static Arc<GpuComputeContext>>> = OnceLock::new();

/// Lazily initialize the global compute context. Returns None when the
/// machine has no usable adapter or during test builds (wgpu Metal backend
/// panics in its destructor during test-runner process teardown on macOS).
pub fn global() -> Option<&'static Arc<GpuComputeContext>> {
    #[cfg(test)]
    {
        // wgpu-core's internal Global static cleans up Metal resources at
        // process exit, which panics because test-runner threads don't hold
        // the Metal main-thread context.  Return None so callers fall back
        // to the CPU path; dedicated GPU tests exercise GpuComputeContext
        // directly.
        None
    }
    #[cfg(not(test))]
    {
        GLOBAL
            .get_or_init(|| {
                GpuComputeContext::new().map(|ctx| &*Box::leak(Box::new(Arc::new(ctx))))
            })
            .as_ref()
            .copied()
    }
...
pub fn try_gpu_color_tint(
    ...
    global()
        .map(|c| c.gpu_color_tint(pixels, w, h, tint_rgb, intensity))
        .unwrap_or(false)
}
pub fn try_gpu_drop_shadow(
    ...
    global()
        .map(|c| c.gpu_drop_shadow(pixels, w, h, color, distance, angle, blur_r))
        .unwrap_or(false)
}
pub fn try_gpu_glow(
    pixels: &mut [u8],
    ...
    global()
        .map(|c| c.gpu_glow(pixels, w, h, threshold, radius, intensity))
        .unwrap_or(false)
}
pub fn try_gpu_levels(
    ...
    global()
        .map(|c| {
            c.gpu_levels(
                pixels, w, h, in_black, in_white, gamma, out_black, out_white,
            )
        })
        .unwrap_or(false)
}
pub fn try_gpu_hue_saturation(
    ...
    global()
        .map(|c| c.gpu_hue_saturation(pixels, w, h, hue_shift, saturation, lightness))
        .unwrap_or(false)
}
pub fn try_gpu_offset(pixels: &mut [u8], w: u32, h: u32, shift_x: i32, shift_y: i32) -> bool {
    ...
    global()
        .map(|c| c.gpu_offset(pixels, w, h, shift_x, shift_y))
        .unwrap_or(false)
}
pub fn try_gpu_twirl(pixels: &mut [u8], w: u32, h: u32, angle: f32, cx: f32, cy: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_twirl(pixels, w, h, angle, cx, cy))
        .unwrap_or(false)
}
pub fn try_gpu_bulge(pixels: &mut [u8], w: u32, h: u32, strength: f32, cx: f32, cy: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_bulge(pixels, w, h, strength, cx, cy))
        .unwrap_or(false)
}
pub fn try_gpu_spherize(
    pixels: &mut [u8],
    w: u32,
    h: u32,
    strength: f32,
    ...
    global()
        .map(|c| c.gpu_spherize(pixels, w, h, strength, cx, cy))
        .unwrap_or(false)
}
pub fn try_gpu_wave_warp(
    ...
    global()
        .map(|c| c.gpu_wave_warp(pixels, w, h, amplitude, frequency, phase, direction))
        .unwrap_or(false)
}
pub fn try_gpu_turbulent_displace(
    pixels: &mut [u8],
    w: u32,
    h: u32,
    amplitude: f32,
    ...
    global()
        .map(|c| c.gpu_turbulent_displace(pixels, w, h, amplitude, scale))
        .unwrap_or(false)
}
pub fn try_gpu_chromatic_aberration(pixels: &mut [u8], w: u32, h: u32, amount: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_chromatic_aberration(pixels, w, h, amount))
        .unwrap_or(false)
}
pub fn try_gpu_vignette(pixels: &mut [u8], w: u32, h: u32, radius: f32, softness: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_vignette(pixels, w, h, radius, softness))
        .unwrap_or(false)
}
pub fn try_gpu_minimax(pixels: &mut [u8], w: u32, h: u32, radius: u32, maximize: bool) -> bool {
    ...
    global()
        .map(|c| c.gpu_minimax(pixels, w, h, radius, maximize))
        .unwrap_or(false)
}
pub fn try_gpu_linear_wipe(
    p: &mut [u8],
    w: u32,
    h: u32,
    completion: f32,
    ...
    global()
        .map(|c| c.gpu_linear_wipe(p, w, h, completion, angle, feather))
        .unwrap_or(false)
}
pub fn try_gpu_shift_channels(
    p: &mut [u8],
    w: u32,
    h: u32,
    r: u32,
    ...
    global()
        .map(|c| c.gpu_shift_channels(p, w, h, r, g, b, a))
        .unwrap_or(false)
}
pub fn try_gpu_color_balance(p: &mut [u8], w: u32, h: u32, shift: [f32; 3]) -> bool {
    ...
    global()
        .map(|c| c.gpu_color_balance(p, w, h, shift))
        .unwrap_or(false)
}
pub fn try_gpu_vibrance(p: &mut [u8], w: u32, h: u32, amount: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_vibrance(p, w, h, amount))
        .unwrap_or(false)
}
pub fn try_gpu_white_balance(p: &mut [u8], w: u32, h: u32, temp: f32, tint: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_white_balance(p, w, h, temp, tint))
        .unwrap_or(false)
}
pub fn try_gpu_hsl_adjust(p: &mut [u8], w: u32, h: u32, hue: f32, sat: f32, light: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_hsl_adjust(p, w, h, hue, sat, light))
        .unwrap_or(false)
}
pub fn try_gpu_crt_scanlines(p: &mut [u8], w: u32, h: u32, spacing: f32, intensity: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_crt_scanlines(p, w, h, spacing, intensity))
        .unwrap_or(false)
}
pub fn try_gpu_alpha_from_luminance(p: &mut [u8], w: u32, h: u32, invert: bool) -> bool {
    ...
    global()
        .map(|c| c.gpu_alpha_from_luminance(p, w, h, invert))
        .unwrap_or(false)
}
pub fn try_gpu_luma_key_range(
    p: &mut [u8],
    w: u32,
    h: u32,
    low: f32,
    ...
    global()
        .map(|c| c.gpu_luma_key_range(p, w, h, low, high, invert))
        .unwrap_or(false)
}
pub fn try_gpu_venetian_blinds(p: &mut [u8], w: u32, h: u32, completion: f32, width: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_venetian_blinds(p, w, h, completion, width))
        .unwrap_or(false)
}
pub fn try_gpu_tritone(
    p: &mut [u8],
    w: u32,
    h: u32,
    shadow: [f32; 3],
    ...
    global()
        .map(|c| c.gpu_tritone(p, w, h, shadow, mid, highlight))
        .unwrap_or(false)
}
pub fn try_gpu_gradient_map(
    p: &mut [u8],
    w: u32,
    h: u32,
    low: [f32; 3],
    ...
    global()
        .map(|c| c.gpu_gradient_map(p, w, h, low, mid, high))
        .unwrap_or(false)
}
pub fn try_gpu_letterbox(p: &mut [u8], w: u32, h: u32, frac: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_letterbox(p, w, h, frac))
        .unwrap_or(false)
}
pub fn try_gpu_cc_lens(p: &mut [u8], w: u32, h: u32, convergence: f32, zoom: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_cc_lens(p, w, h, convergence, zoom))
        .unwrap_or(false)
}
pub fn try_gpu_polar_coords(p: &mut [u8], w: u32, h: u32, to_polar: bool) -> bool {
    ...
    global()
        .map(|c| c.gpu_polar_coords(p, w, h, to_polar))
        .unwrap_or(false)
}
pub fn try_gpu_optics_comp(p: &mut [u8], w: u32, h: u32, fov: f32, reverse: bool) -> bool {
    ...
    global()
        .map(|c| c.gpu_optics_comp(p, w, h, fov, reverse))
        .unwrap_or(false)
}
pub fn try_gpu_fisheye(p: &mut [u8], w: u32, h: u32, strength: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_fisheye(p, w, h, strength))
        .unwrap_or(false)
}
pub fn try_gpu_lens_correction(p: &mut [u8], w: u32, h: u32, k1: f32, k2: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_lens_correction(p, w, h, k1, k2))
        .unwrap_or(false)
}
pub fn try_gpu_vortex(p: &mut [u8], w: u32, h: u32, angle: f32, radius: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_vortex(p, w, h, angle, radius))
        .unwrap_or(false)
}
pub fn try_gpu_pinch_punch(p: &mut [u8], w: u32, h: u32, strength: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_pinch_punch(p, w, h, strength))
        .unwrap_or(false)
}
pub fn try_gpu_refraction(p: &mut [u8], w: u32, h: u32, ior: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_refraction(p, w, h, ior))
        .unwrap_or(false)
}
pub fn try_gpu_bend_it(p: &mut [u8], w: u32, h: u32, top: f32, bottom: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_bend_it(p, w, h, top, bottom))
        .unwrap_or(false)
}
pub fn try_gpu_tiler(p: &mut [u8], w: u32, h: u32, scale: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_tiler(p, w, h, scale))
        .unwrap_or(false)
}
pub fn try_gpu_directional_sharpen(
    p: &mut [u8],
    w: u32,
    h: u32,
    angle: f32,
    ...
    global()
        .map(|c| c.gpu_directional_sharpen(p, w, h, angle, strength))
        .unwrap_or(false)
}
pub fn try_gpu_halftone(p: &mut [u8], w: u32, h: u32, cell_size: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_halftone(p, w, h, cell_size))
        .unwrap_or(false)
}
pub fn try_gpu_mosaic(p: &mut [u8], w: u32, h: u32, bw: f32, bh: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_mosaic(p, w, h, bw, bh))
        .unwrap_or(false)
}
pub fn try_gpu_crosshatch(p: &mut [u8], w: u32, h: u32, gap: f32, threshold: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_crosshatch(p, w, h, gap, threshold))
        .unwrap_or(false)
}
pub fn try_gpu_colorama(
    p: &mut [u8],
    w: u32,
    h: u32,
    hue_offset: f32,
    ...
    global()
        .map(|c| c.gpu_colorama(p, w, h, hue_offset, sat, light))
        .unwrap_or(false)
}
pub fn try_gpu_bevel_alpha(p: &mut [u8], w: u32, h: u32, depth: f32, angle: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_bevel_alpha(p, w, h, depth, angle))
        .unwrap_or(false)
}
pub fn try_gpu_chroma_key(
    p: &mut [u8],
    w: u32,
    h: u32,
    key_rgb: [f32; 3],
    ...
    global()
        .map(|c| c.gpu_chroma_key(p, w, h, key_rgb, gain, softness))
        .unwrap_or(false)
}
pub fn try_gpu_color_space_convert(p: &mut [u8], w: u32, h: u32, mode: u32) -> bool {
    ...
    global()
        .map(|c| c.gpu_color_space_convert(p, w, h, mode))
        .unwrap_or(false)
}
pub fn try_gpu_film_grain(p: &mut [u8], w: u32, h: u32, intensity: f32, seed: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_film_grain(p, w, h, intensity, seed))
        .unwrap_or(false)
}
pub fn try_gpu_fractal_noise(
    p: &mut [u8],
    w: u32,
    h: u32,
    scale: f32,
    ...
    global()
        .map(|c| c.gpu_fractal_noise(p, w, h, scale, evolution, blend))
        .unwrap_or(false)
}
pub fn try_gpu_glitch_displacement(p: &mut [u8], w: u32, h: u32, amount: f32, seed: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_glitch_displacement(p, w, h, amount, seed))
        .unwrap_or(false)
}
pub fn try_gpu_scanline_glitch(p: &mut [u8], w: u32, h: u32, amount: f32, seed: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_scanline_glitch(p, w, h, amount, seed))
        .unwrap_or(false)
}
pub fn try_gpu_reflection_map(p: &mut [u8], w: u32, h: u32, reflect_y: bool, fade: f32) -> bool {
    ...
    global()
        .map(|c| c.gpu_reflection_map(p, w, h, reflect_y, fade))
        .unwrap_or(false)
}
