...
impl BlenderCamTrack {
    ...
    pub fn apply_to_comp(&self, comp: &mut Composition, match_comp_fps: bool) -> usize {
        ...
        for tf in &self.frames {
            let f = ((tf.frame as f32 * rate).round() as u32)
                .min(comp.duration_frames.saturating_sub(1));
            let mut p = [
                tf.pos[0] as f32 * self.scale,
        ...
        let cam: &mut Camera3D = comp.resolve_camera_mut();
        cam.active = true;
        if !pos_kfs.is_empty() {
...
/// Exports the composition's active 3D camera track to a Blender-compatible JSON interchange string.
pub fn export_comp_camera_to_blender_json(comp: &Composition) -> Result<String, String> {
    let cam = comp.resolve_camera();
    let duration = comp.duration_frames.max(1);

    let mut frames = Vec::with_capacity(duration as usize);

    for f in 0..duration {
        let p = cam.transform.position.evaluate(f);
        let r = cam.transform.rotation.evaluate(f);

        // App Y-up -> Blender Z-up: (x, y, z) -> (x, z, -y)
        let blender_pos = [p[0] as f64, p[2] as f64, -p[1] as f64];
        let blender_rot = [r[0] as f64, r[2] as f64, -r[1] as f64];
        let fov = cam.fov_at(f) as f64;
    ...
    serde_json::to_string_pretty(&payload).map_err(|e| format!("Failed to serialize camera track: {}", e))
}

/// Generates a ready-to-run Blender Python script (`import_camera.py`) to import the exported camera into Blender.
pub fn generate_blender_import_script(json_file_path: &str) -> String {
    format!(
r#"# Blender Camera Import Script (Auto-generated by After Effects OSS Alternative)
import bpy
import json
import math
...
scene.render.fps = data.get('fps', 30)

cam_data = bpy.data.cameras.new(name="AE_Imported_Camera")
cam_obj = bpy.data.objects.new(name="AE_Imported_Camera", object_data=cam_data)
...
for frame_data in data.get('frames', []):
    f = frame_data['frame']
    pos = frame_data['pos']
    rot = frame_data.get('rot_deg', [0, 0, 0])
    ...
    if 'fov' in frame_data and frame_data['fov'] is not None:
        cam_data.angle = math.radians(frame_data['fov'])
        cam_data.keyframe_insert(data_path="angle", frame=f)
        
    cam_obj.keyframe_insert(data_path="location", frame=f)
    cam_obj.keyframe_insert(data_path="rotation_euler", frame=f)

print(f"Successfully imported {{len(data.get('frames', []))}} camera frames from AE!")
"#,
...
mod tests {
    ...
    fn test_fps_rescale_matches_comp() {
        ...
        assert_eq!(
            kfs.last().unwrap().frame,
            20,
            "frame 10 @24fps -> 20 @48fps"
        );
    }
    ...
    fn test_solve_3d_camera_from_tracks() {
        ...
        let solve = solve_3d_camera_from_tracks(&features, 1920, 1080, 50.0)
            .expect("camera solve succeeds");
    ...
    #[test]
    fn test_export_comp_camera_to_blender_json_and_script() {
        let comp = Composition::new("c".into(), "C".into(), 1920, 1080, 30, 60);
        let json = export_comp_camera_to_blender_json(&comp).expect("export json succeeds");
        assert!(json.contains("\"fps\": 30"));
        assert!(json.contains("\"frames\": ["));

        let py = generate_blender_import_script("/tmp/test_cam.json");
        assert!(py.contains("import bpy"));
        assert!(py.contains("/tmp/test_cam.json"));
    }
}
