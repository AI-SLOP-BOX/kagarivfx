...
pub fn apply_chromatic_aberration(
    ...
    let Some(expected) = rgba_buffer_size(width, height) else { return };
...
pub fn apply_vignette(
    ...
    pixels
        ...
        .for_each(|(i, chunk)| {
            ...
            let ry = dy;
            let norm_dist =
                ((rx * rx + ry * ry).powf(roundness.clamp(0.1, 10.0) * 0.5)).min(1.0);
...
pub fn apply_color_grade_lut(
    ...
    pixels
        ...
        .for_each(|chunk| {
            ...
            let fetch_lut = |xi: usize, yi: usize, zi: usize| -> [f32; 3] {
                lut_data[idx3d(xi, yi, zi)]
            };
...
pub fn apply_color_space_convert(
    pixels: &mut [u8],
    _width: u32,
    _height: u32,
    mode: u32,
...
pub fn apply_fractal_noise(
    ...
    fractal_type: f32, contrast: f32, brightness: f32,
    complexity: f32, evolution: f32,
) {
    let Some(expected) = rgba_buffer_size(width, height) else { return };
    if pixels.len() < expected {
        return;
        ...
        for px in 0..width {
            ...
            match ft {
                ...
                1 => {
                    ...
                    let mut freq = 1.0f32;
                    for o in 0..octaves {
                        n += noise_2d((nx + evo) * freq, (ny + evo) * freq, seed.wrapping_add(o * 1000)).abs() * amp;
                        amp *= 0.5;
                        freq *= 2.0;
                    }
                    n = n.clamp(0.0, 1.0);
                ...
                2 => {
                    ...
                    for o in 0..octaves {
                        ...
                        n += noise_2d((nx + ev) * freq, (ny + ev * 0.7) * freq, seed.wrapping_add(o * 1000)).abs() * amp;
                        amp *= 0.5;
                        freq *= 2.0;
                    }
                    n = n.clamp(0.0, 1.0);
                ...
                3 => {
                    ...
                    let mut freq = 1.0f32;
                    for o in 0..octaves {
                        let val = noise_2d((nx + evo) * freq, (ny + evo) * freq, seed.wrapping_add(o * 1000));
...
pub fn apply_curves(
    pixels: &mut [u8], _width: u32, _height: u32,
    channel: f32,
) {
...
pub fn apply_displacement_map(
    pixels: &mut [u8], width: u32, height: u32,
    _source_layer: f32, max_horizontal: f32, max_vertical: f32,
) {
    ...
    for py in 0..height {
        for px in 0..width {
            let idx = ((py * width + px) * 4) as usize;
            if idx + 3 >= orig.len() { continue; }

            // Generate a simple displacement pattern (sine wave)
            let disp_x = ((px as f32 / w * std::f32::consts::TAU + py as f32 / h * std::f32::consts::PI).sin() * 0.5 + 0.5) * max_horizontal;
            let disp_y = ((py as f32 / h * std::f32::consts::TAU + px as f32 / w * std::f32::consts::PI).cos() * 0.5 + 0.5) * max_vertical;
...
pub fn apply_compound_blur(
    pixels: &mut [u8], width: u32, height: u32,
    _source_layer: f32, max_blur: f32,
) {
    if max_blur < 0.5 { return; }
            ...
            for px in 0..width {
                ...
                for dy in -(r as i32)..=r as i32 {
                    for dx in -(r as i32)..=r as i32 {
                        if dx * dx + dy * dy > (r as i32 * r as i32) { continue; }
...
pub fn apply_minimax(
    pixels: &mut [u8], width: u32, height: u32,
    operation: f32, radius: f32,
) {
            ...
            for px in 0..width {
                ...
                for dy in -(r as i32)..=r as i32 {
                    for dx in -(r as i32)..=r as i32 {
                        ...
                        let sidx = ((sy * width + sx) * 4) as usize;
                        if sidx + 3 < orig.len() {
                            let lum = (orig[sidx] as f32 + orig[sidx + 1] as f32 + orig[sidx + 2] as f32) / 3.0;
                            if use_max { best = best.max(lum); } else { best = best.min(lum); }
                        }
                    }
                }
                let v = best as u8;
...
pub fn apply_shift_channels(
    pixels: &mut [u8], _width: u32, _height: u32,
    take_red: f32, take_green: f32, take_blue: f32, take_alpha: f32,
) {
    ...
    for px in pixels.chunks_exact_mut(4) {
        ...
        px[0] = match src_r { 0..=3 => orig[src_r], 4 => 0, 5 => 255, _ => orig[0] };
