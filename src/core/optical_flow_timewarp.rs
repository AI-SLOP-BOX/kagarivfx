...
pub fn detect_markerless_features(
    ...
    if width < 5 || height < 5 || frame_rgba.len() != size || max_features == 0 {
        return Vec::new();
    }
    let luma = |x: u32, y: u32| -> f32 {
        let i = (y as usize * width as usize + x as usize) * 4;
        0.299 * frame_rgba[i] as f32
            + 0.587 * frame_rgba[i + 1] as f32
            + 0.114 * frame_rgba[i + 2] as f32
    };
    ...
    for (_, point) in candidates {
        if selected
            .iter()
            .all(|other: &[f32; 2]| (point[0] - other[0]).hypot(point[1] - other[1]) >= spacing)
        {
            selected.push(point);
...
pub fn assign_features_to_humanoid(
    ...
    assign_features_to_humanoid_with_confidence(features, width, height, 1.0)
        .into_iter()
        .map(|value| value.map(|(index, _)| index))
        .collect()
}
...
pub fn assign_features_to_humanoid_with_confidence(
    ...
    if features.is_empty() || width == 0 || height == 0 {
        return vec![None; 17];
    }
    let minimum_confidence = if minimum_confidence.is_finite() {
    ...
    canonical
        ...
        .map(|&(x, y)| {
            features
                ...
                .map(|(index, point)| {
                    let da =
                        ((point[0] - min_x) / span_x - x).hypot((point[1] - min_y) / span_y - y);
                    (index, point, da)
                })
                .min_by(|(_, _, da), (_, _, db)| da.total_cmp(db))
                .and_then(|(index, _, distance)| {
...
pub fn track_markerless_motion(
    ...
    for frame in 1..frames.len() {
        ...
        for (index, position) in positions.iter_mut().enumerate() {
            ...
            let reverse =
                reverse_flow.get(predicted[0].round() as u32, predicted[1].round() as u32);
            ...
            *position = if confidence < 0.15 {
                redetect_markerless_feature(
                    frames[frame],
                    width,
                    height,
                    predicted,
                    search_radius.max(1) as u32,
                )
                .unwrap_or(predicted)
            } else {
                predicted
...
pub fn estimate_markerless_pose(
    ...
    for frame_index in 0..frame_count {
        ...
        for (joint, assignment) in assignments.iter().enumerate() {
            if let Some(track_index) = assignment {
                if let Some(sample) = tracks
                    .get(*track_index)
                    .and_then(|track| track.samples.get(frame_index))
                {
    ...
    let mut pose = MarkerlessPoseTrack {
        frames: pose_frames,
        bones: standard_humanoid_bones(17),
        bone_lengths: Vec::new(),
    };
    pose.bone_lengths = pose
        .bones
        .iter()
        .map(|bone| {
            pose.frames
...
pub fn smooth_markerless_track(
    ...
    for (index, sample) in track.samples.iter().enumerate() {
        ...
        let position = if weight_sum > 0.0 {
            [
                weighted_position[0] / weight_sum,
                weighted_position[1] / weight_sum,
            ]
        } else {
            sample.position
        };
...
pub fn pose_joint_velocities(pose: &MarkerlessPoseTrack, frame_index: usize) -> Vec<[f32; 2]> {
    let Some(current) = pose.frames.get(frame_index) else {
        return Vec::new();
    };
    ...
    current
        ...
        .enumerate()
        .map(|(index, point)| {
            let Some(previous_point) = previous.and_then(|frame| frame.joints.get(index)) else {
                return [0.0, 0.0];
            };
            if point.iter().all(|value| value.is_finite())
                && previous_point.iter().all(|value| value.is_finite())
            {
...
pub fn pose_root_acceleration(pose: &MarkerlessPoseTrack, frame_index: usize) -> [f32; 2] {
    if frame_index < 2 || frame_index >= pose.frames.len() {
        return [0.0, 0.0];
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
pub enum MarkerlessMotionEventKind {
    Peak,
    Stop,
    DirectionChange,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
pub struct MarkerlessMotionEvent {
    pub frame: u32,
    pub joint: usize,
    pub kind: MarkerlessMotionEventKind,
    pub magnitude: f32,
}
...
pub fn detect_markerless_motion_events(
    ...
    let speed_threshold = if speed_threshold.is_finite() && speed_threshold >= 0.0 {
        speed_threshold
    } else {
        0.0
    };
    let acceleration_threshold =
        if acceleration_threshold.is_finite() && acceleration_threshold >= 0.0 {
    ...
    for frame_index in 1..pose.frames.len() {
        ...
        for (joint, velocity) in velocities.iter().enumerate() {
            ...
            if speed >= speed_threshold && speed.is_finite() {
                events.push(MarkerlessMotionEvent {
                    frame: pose.frames[frame_index].frame,
                    joint,
                    kind: MarkerlessMotionEventKind::Peak,
                    magnitude: speed,
                });
            }
            if frame_index > 1 {
                let previous = pose_joint_velocities(pose, frame_index - 1)
                    .get(joint)
                    .copied()
                    .unwrap_or([0.0, 0.0]);
                ...
                if magnitude >= acceleration_threshold && magnitude.is_finite() {
                    ...
                    let kind = if dot < 0.0 {
                        MarkerlessMotionEventKind::DirectionChange
                    } else {
                        MarkerlessMotionEventKind::Peak
                    };
                    events.push(MarkerlessMotionEvent {
                        frame: pose.frames[frame_index].frame,
                        joint,
            ...
            if speed <= speed_threshold && frame_index > 1 {
                events.push(MarkerlessMotionEvent {
                    frame: pose.frames[frame_index].frame,
                    joint,
                    kind: MarkerlessMotionEventKind::Stop,
                    magnitude: speed,
                });
            }
        }
...
pub fn rgba_to_normalized_rgb(frame_rgba: &[u8], width: u32, height: u32) -> Option<Vec<f32>> {
    let byte_len = (width as usize)
        .checked_mul(height as usize)?
        .checked_mul(4)?;
    if width == 0 || height == 0 || frame_rgba.len() != byte_len {
        return None;
    }
...
pub fn project_pose3d_point_with_rotation(
    ...
    if !point.iter().all(|value| value.is_finite())
        ...
        || !camera.principal_point.iter().all(|value| value.is_finite())
...
pub fn project_pose3d_track_to_motion_tracks(
    ...
    let joint_count = pose
        ...
        .unwrap_or(0);
    (0..joint_count)
        .map(|joint| MarkerlessMotionTrack {
            samples: pose
                .frames
                .iter()
                .filter_map(|frame| {
                    ...
                    let position =
                        project_pose3d_point_with_rotation(point, camera, rotation_degrees)?;
                    Some(MarkerlessMotionSample {
                        frame: frame.frame,
                        position,
...
pub fn markerless_pose3d_to_csv(pose: &MarkerlessPose3DTrack) -> String {
    ...
    for name in &names {
        output.push_str(&format!(",{}_x,{}_y,{}_z", name, name, name));
    }
    output.push('\n');
...
pub fn markerless_pose3d_to_json(
    pose: &MarkerlessPose3DTrack,
) -> Result<String, serde_json::Error> {
    serde_json::to_string_pretty(pose)
}

pub fn stabilize_markerless_pose3d_track(
    pose: &mut MarkerlessPose3DTrack,
    max_gap: usize,
    radius: usize,
) -> usize {
    if pose.frames.is_empty() || max_gap == 0 {
        return 0;
    }
    let count = pose
        .frames
    ...
    for joint in 0..count {
        ...
        while index < pose.frames.len() {
            if pose.frames[index].joints[joint]
                .iter()
                .all(|value| value.is_finite())
            {
                index += 1;
            ...
            let start = index;
            while index < pose.frames.len()
                && !pose.frames[index].joints[joint]
                    .iter()
                    .all(|value| value.is_finite())
            {
                index += 1;
            ...
            let end = index;
            if start == 0 || end >= pose.frames.len() {
                continue;
            }
            let gap = pose.frames[end]
                .frame
                .saturating_sub(pose.frames[start - 1].frame)
                .saturating_sub(1) as usize;
            if gap > max_gap || pose.frames[end].frame <= pose.frames[start - 1].frame {
                continue;
            }
...
pub fn pose3d_quality(pose: &MarkerlessPose3DTrack, frame_index: usize) -> (usize, f32) {
    let Some(frame) = pose.frames.get(frame_index) else {
        return (0, 0.0);
    };
    let valid = frame
        .joints
    ...
    (
        valid,
        if frame.confidence.is_finite() {
            frame.confidence.clamp(0.0, 1.0)
        } else {
            0.0
        },
    )
...
pub fn normalize_pose3d_bones(pose: &mut MarkerlessPose3DTrack) -> usize {
    ...
    pose.bones
        .retain(|bone| bone[0] < joint_count && bone[1] < joint_count && bone[0] != bone[1]);
...
pub fn estimate_pose_3d_with_backend<B: Pose3DInferenceBackend>(
    ...
    let normalized = frames
        .iter()
        .map(|frame| rgba_to_normalized_rgb(frame, width, height))
        .collect::<Option<Vec<_>>>();
    let Some(normalized) = normalized else {
        return MarkerlessPose3DTrack {
            frames: Vec::new(),
            bones: Vec::new(),
        };
    };
    ...
    for (frame_index, rgb) in normalized.iter().enumerate() {
        let joints = backend
            .infer_joints_3d(rgb, width, height)
            .into_iter()
            .take(17)
            ...
            .collect::<Vec<_>>();
        let confidence = if joints.is_empty() {
            0.0
        } else {
            joints.iter().map(|(_, value)| *value).sum::<f32>() / joints.len() as f32
        };
        outputs.push(MarkerlessPose3DFrame {
            frame: frame_index as u32,
            joints: joints.into_iter().map(|(point, _)| point).collect(),
            confidence,
        });
    }
    let count = outputs
    ...
    let mut pose = MarkerlessPose3DTrack {
        frames: outputs,
        bones: standard_humanoid_bones(count),
    };
...
pub fn estimate_pose_with_normalized_backend<B: NormalizedPoseInferenceBackend>(
    ...
    let mut pose_backend = RawPoseResultsBackend {
        results: &raw_refs,
        index: 0,
    };
...
struct RawPoseResultsBackend<'a> {
    results: &'a [&'a [([f32; 2], f32)]],
    index: usize,
}
...
pub fn estimate_pose_with_backend<B: PoseInferenceBackend>(
    ...
    for frame in frames {
        let joints = backend
            .infer_joints(frame, width, height)
            .into_iter()
            .take(17)
            .map(|(point, confidence)| {
                if point.iter().all(|value| value.is_finite()) && confidence.is_finite() {
...
pub fn pose_quality(pose: &MarkerlessPoseTrack, frame_index: usize) -> (usize, f32) {
    ...
    let valid = frame
        ...
        .filter(|point| point.iter().all(|value| value.is_finite()))
        .count();
    let confidence = if frame.joints.is_empty() {
        0.0
    } else {
        frame.confidence.clamp(0.0, 1.0)
    };
...
pub fn name_markerless_pose_track(
    ...
    let names = joint_names
        ...
        .map(|name| normalize_joint_name(name))
        .collect::<Vec<_>>();
        ...
        .map(|bone| {
            pose.bone_lengths
                .get(
                    pose.bones
                        .iter()
                        .position(|candidate| candidate[0] == bone[0] && candidate[1] == bone[1])
                        .unwrap_or(usize::MAX),
                )
                .copied()
                .unwrap_or_else(|| {
...
pub fn repair_markerless_pose_track(pose: &mut MarkerlessPoseTrack, max_gap: usize) -> usize {
    if pose.frames.is_empty() || max_gap == 0 {
        return 0;
    }
    let joint_count = pose
    ...
    for joint in 0..joint_count {
        ...
        while index < pose.frames.len() {
            ...
            for offset in 0..(end - start) {
                ...
                if current_frame <= before_frame || current_frame >= after_frame {
                    continue;
                }
                let t = if span > 0.0 {
                    current_frame.saturating_sub(before_frame) as f32 / span
                } else {
                    0.0
                };
                ...
                pose.frames[start + offset].confidence =
                    (pose.frames[start + offset].confidence * 0.5).clamp(0.0, 1.0);
                repaired += 1;
            }
        }
    }
...
pub fn stabilize_markerless_pose_track(
    ...
    if pose.frames.is_empty() || radius == 0 {
        return repaired;
    }
    let joint_count = pose
        .frames
    ...
    for frame_index in 0..pose.frames.len() {
        ...
        for joint in 0..joint_count {
            ...
            if pose.frames[frame_index].joints[joint] != smoothed {
                pose.frames[frame_index].joints[joint] = smoothed;
                pose.frames[frame_index].confidence =
                    (pose.frames[frame_index].confidence * 0.98).clamp(0.0, 1.0);
                changed += 1;
            }
        }
    }
...
pub fn reject_markerless_motion_outliers(
    ...
    if track.samples.len() < 2 || !max_step.is_finite() || max_step <= 0.0 {
        return 0;
    }
    let mut changed = 0;
    for index in 1..track.samples.len() {
        ...
        if !distance.is_finite() || distance > max_step {
            ...
            track.samples[index].confidence =
                (track.samples[index].confidence * 0.25).clamp(0.0, 1.0);
            changed += 1;
        }
    }
    changed
...
pub fn markerless_pose_to_csv(pose: &NamedMarkerlessPoseTrack) -> String {
    ...
    for frame in &pose.frames {
        output.push_str(&format!(
            "{},{},{},{}",
            frame.frame, frame.root[0], frame.root[1], frame.confidence
        ));
...
pub fn markerless_pose_to_bvh(pose: &NamedMarkerlessPoseTrack, frame_time: f32) -> String {
    ...
    let frame_time = if frame_time.is_finite() && frame_time > 0.0 {
        frame_time
    } else {
        1.0 / 30.0
    };
    ...
    for bone in &pose.bones {
        if bone[1] < parent.len() && parent[bone[1]].is_none() {
            parent[bone[1]] = Some(bone[0]);
        }
    }
    ...
    fn write_joint(
        ...
        names: &[String],
        children: &[Vec<usize>],
        frame: &MarkerlessPoseFrame,
        is_root: bool,
        visited: &mut Vec<bool>,
    ) {
        if index >= names.len() || visited[index] {
            return;
        }
        visited[index] = true;
        ...
        let parent_point = parent
            .and_then(|p| frame.joints.get(p).copied())
            .unwrap_or([0.0, 0.0]);
        let (ox, oy) = if is_root {
            (0.0, 0.0)
        } else {
            (point[0] - parent_point[0], point[1] - parent_point[1])
        };
    ...
    for (frame_index, frame) in pose.frames.iter().enumerate() {
        ...
        output.push_str(&format!(
            "{:.6} {:.6} 0.000000",
            root_point[0], root_point[1]
        ));
        for index in 0..pose.joint_names.len() {
            if index == root {
                continue;
            }
            let angle = parent[index]
                .and_then(|p| {
                    let a = initial.get(index)?;
                    let b = initial.get(p)?;
                    let ia = frame.joints.get(index)?;
                    let ib = frame.joints.get(p)?;
                    Some(
                        ((ia[1] - ib[1]).atan2(ia[0] - ib[0]) - (a[1] - b[1]).atan2(a[0] - b[0]))
                            .to_degrees(),
                    )
                })
                .unwrap_or(0.0);
        ...
        if frame_index + 1 < pose.frames.len() {
            output.push('\n');
        }
    }
    output
...
pub fn build_markerless_pose_track(
    ...
    for frame_index in 0..frame_count {
        let joints = joint_tracks
            .iter()
            .map(|track| track.samples[frame_index].position)
            .collect::<Vec<_>>();
        let root = joints.iter().fold([0.0, 0.0], |sum, point| {
            [sum[0] + point[0], sum[1] + point[1]]
        });
        let root = [root[0] / joints.len() as f32, root[1] / joints.len() as f32];
        let confidence = joint_tracks
            .iter()
            .map(|track| track.samples[frame_index].confidence)
            .sum::<f32>()
...
pub fn markerless_pose_to_json(pose: &MarkerlessPoseTrack) -> Result<String, serde_json::Error> {
    serde_json::to_string_pretty(pose)
}
...
mod tests {
    ...
    fn pose_track_uses_common_frames_and_computes_bones() {
        ...
        assert_eq!(
            normalize_joint_name(" Left Shoulder-01 "),
            "left_shoulder_01"
        );
    ...
    fn feature_detection_is_bounded_spaced_and_deterministic() {
        ...
        assert!(first
            .iter()
            .all(|p| p[0] >= 1.0 && p[0] < 11.0 && p[1] >= 1.0 && p[1] < 11.0));
        let refs = [&frame[..]];
        assert_eq!(
            track_markerless_auto(&refs, w, h, 2, 2, 1, 1).len(),
            first.len().min(2)
        );
        ...
        assert_eq!(
            assigned.len(),
            assignments.iter().filter(|value| value.is_some()).count()
        );
    }

    #[test]
    fn motion_outlier_is_clamped_and_marked_low_confidence() {
        let mut track = MarkerlessMotionTrack {
            samples: vec![
                MarkerlessMotionSample {
                    frame: 0,
                    position: [0.0, 0.0],
    ...
    fn pose_motion_metrics_are_finite_and_deterministic() {
        ...
        assert!(events
            .iter()
            .any(|event| event.kind == MarkerlessMotionEventKind::Peak));
    ...
    impl NormalizedPoseInferenceBackend for TestNormalizedPoseBackend {
        fn infer_normalized_rgb(
            ...
            if rgb.iter().all(|value| (0.0..=1.0).contains(value)) {
                vec![([1.0, 1.0], 1.0)]
            } else {
                Vec::new()
            }
    ...
    fn pose_3d_track_projection_returns_editable_motion_tracks() {
        ...
        let tracks = project_pose3d_track_to_motion_tracks(
            &pose,
            PoseCameraModel {
                focal_length: 100.0,
                principal_point: [50.0, 40.0],
    ...
    fn pose_3d_csv_preserves_joint_names_and_depth() {
        let pose = MarkerlessPose3DTrack {
            frames: vec![MarkerlessPose3DFrame {
                frame: 4,
                joints: vec![[1.0, 2.0, 3.0]],
                confidence: 0.75,
            }],
            bones: vec![],
        };
    ...
    fn pose_3d_quality_counts_only_finite_joints() {
        let pose = MarkerlessPose3DTrack {
            frames: vec![MarkerlessPose3DFrame {
                frame: 0,
                joints: vec![[1.0, 2.0, 3.0], [f32::NAN; 3]],
                confidence: 1.5,
            }],
            bones: vec![],
        };
