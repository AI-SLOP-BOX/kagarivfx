...
fn dirs_home() -> Option<std::path::PathBuf> {
    std::env::var_os("HOME").map(std::path::PathBuf::from).or_else(|| std::env::var_os("USERPROFILE").map(std::path::PathBuf::from))
}
...
pub fn save_mode(mode: UiMode) {
    ...
    let s = UiSettings { ui_mode_advanced: mode.is_advanced() };
...
pub fn load_mode() -> UiMode {
    let Some(path) = settings_path() else { return UiMode::Beginner };
    match std::fs::read_to_string(path) {
        Ok(json) => {
            let s: UiSettings = serde_json::from_str(&json).unwrap_or_default();
            if s.ui_mode_advanced { UiMode::Advanced } else { UiMode::Beginner }
        }
