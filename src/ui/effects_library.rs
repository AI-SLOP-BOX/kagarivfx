...
pub fn draw(app: &mut KagariApp, ctx: &egui::Context, current_frame: &mut u32) {
    ...
    egui::SidePanel::right("right_panel")
        ...
        .show(ctx, |ui| {
            ...
            // Lazy pre-edit snapshot: captured on first mutation only (zero clones while idle).
            let mut pre_edit_snapshot: Option<Project> = None;
            ...
            if app.ui_tabs.right_tab_idx == 3 {
                ...
                // Lazy snapshot: capture pre-edit state only once, before any mutation
                if pre_edit_snapshot.is_none() {
                    pre_edit_snapshot = Some(app.history.current().clone());
                }
