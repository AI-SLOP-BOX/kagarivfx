...
pub fn draw_proxy_controls(app: &mut AfterEffectsApp, ui: &mut egui::Ui) {
    let (current, active_in_preview) = {
        ...
        (comp.comp_proxy.global_resolution, comp.comp_proxy.active_in_preview)
    };

    ui.horizontal(|ui| {
        ui.label(egui::RichText::new("Proxy").small().color(colors::TEXT_PRIMARY));
        ...
        let btn = |ui: &mut egui::Ui, label: &str, res: ProxyResolution| {
            ...
            ui.selectable_label(active,
                egui::RichText::new(label).small().color(
                    if active { colors::ACCENT_CYAN } else { colors::TEXT_PRIMARY }
                )
            )
        };

        if btn(ui, "Full", ProxyResolution::Full).clicked() {
            app.history.current_mut().active_composition_mut().comp_proxy.global_resolution = ProxyResolution::Full;
        }
        if btn(ui, "Half", ProxyResolution::Half).clicked() {
            app.history.current_mut().active_composition_mut().comp_proxy.global_resolution = ProxyResolution::Half;
        }
        if btn(ui, "¼", ProxyResolution::Quarter).clicked() {
            app.history.current_mut().active_composition_mut().comp_proxy.global_resolution = ProxyResolution::Quarter;
        }
        if btn(ui, "⅛", ProxyResolution::Eighth).clicked() {
            app.history.current_mut().active_composition_mut().comp_proxy.global_resolution = ProxyResolution::Eighth;
        }

        ui.separator();

        if ui.selectable_label(active_in_preview,
            egui::RichText::new(if active_in_preview { "Proxy ON" } else { "Proxy OFF" }).small().color(
                if active_in_preview { colors::ACCENT_CYAN } else { colors::TEXT_MUTED }
            )
        ).clicked() {
...
pub fn draw_layer_proxy(app: &mut AfterEffectsApp, ui: &mut egui::Ui, layer_idx: usize) {
    let (enabled, res) = {
        let comp = app.history.current().active_composition();
        if layer_idx >= comp.layers.len() { return; }
        let proxy = &comp.layers[layer_idx].proxy;
        (proxy.enabled, proxy.resolution)
    };

    ui.horizontal(|ui| {
        ui.label(egui::RichText::new("Proxy").small().color(colors::TEXT_MUTED));

        if ui.checkbox(&mut enabled.clone(), "").changed() {
            app.history.current_mut().active_composition_mut().layers[layer_idx].proxy.enabled = !enabled;
        }
        ...
        if enabled {
            ...
            if btn(ui, "½", ProxyResolution::Half).clicked() {
                app.history.current_mut().active_composition_mut().layers[layer_idx].proxy.resolution = ProxyResolution::Half;
            }
            if btn(ui, "¼", ProxyResolution::Quarter).clicked() {
                app.history.current_mut().active_composition_mut().layers[layer_idx].proxy.resolution = ProxyResolution::Quarter;
            }
            if btn(ui, "⅛", ProxyResolution::Eighth).clicked() {
                app.history.current_mut().active_composition_mut().layers[layer_idx].proxy.resolution = ProxyResolution::Eighth;
            }
        }
    });
}
