...
pub fn draw_speed_graph_options(app: &mut AfterEffectsApp, ui: &mut egui::Ui, current_frame: u32) {
    ...
    let (mut in_influence, mut in_speed, mut out_influence, mut out_speed) = ui.ctx().data_mut(|d| {
    ...
    ui.ctx().data_mut(|d| d.insert_temp(vel_id, (in_influence, in_speed, out_influence, out_speed)));

    ui.add_space(8.0);
    ui.separator();
    ui.horizontal(|ui| {
        if ui.button("Apply Velocity").on_hover_text("Write the velocity/influence values into the selected track's keyframes").clicked() {
            let Some(layer_idx) = app.selected_layer_idx else {
                app.toasts.error("Select a layer first");
                return;
            };
            let prop = app.selected_property.clone().unwrap_or_else(|| "Position X".into());
            app.modify_project(|p| {
                ...
                match prop.as_str() {
                    "Position X" | "Position Y" => apply_velocity_bezier(&mut t.position, in_influence, in_speed, out_influence, out_speed),
                    "Scale X" | "Scale Y" => apply_velocity_bezier(&mut t.scale, in_influence, in_speed, out_influence, out_speed),
                    "Rotation" => apply_velocity_bezier(&mut t.rotation, in_influence, in_speed, out_influence, out_speed),
                    "Opacity" => apply_velocity_bezier(&mut t.opacity, in_influence, in_speed, out_influence, out_speed),
                    _ => {}
                }
            });
            app.toasts.info(format!("Velocity applied to {}", prop));
        }
        if ui.button("Easy Ease (F9)").on_hover_text("Symmetrical smooth ease on all keyframes").clicked() {
            apply_preset_ease(app, 33.3, 0.0, 33.3, 0.0, |x1, _y1, x2, _y2| [x1, 0.1, x2, 1.0]);
        }
        if ui.button("Easy Ease In").on_hover_text("Ease into the next keyframe").clicked() {
            apply_preset_ease(app, 33.3, 0.0, 33.3, 0.0, |_x1, _y1, x2, _y2| [0.0, 0.0, x2, 1.0]);
        }
        if ui.button("Easy Ease Out").on_hover_text("Ease out of the previous keyframe").clicked() {
            apply_preset_ease(app, 33.3, 0.0, 33.3, 0.0, |x1, _y1, _x2, _y2| [x1, 0.1, 1.0, 1.0]);
        }
    });
    let _ = current_frame;
}
...
fn apply_preset_ease(
    ...
    in_inf: f32, _in_spd: f32, out_inf: f32, _out_spd: f32,
