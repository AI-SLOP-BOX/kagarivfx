...
pub fn draw_marker_panel(app: &mut AfterEffectsApp, ui: &mut egui::Ui, current_frame: u32) {
    ...
    ui.horizontal(|ui| {
        if ui.button("📍 Add Marker at Current Time (*)").on_hover_text("Adds marker at current playhead frame").clicked() {
            let mut temp_proj = app.history.current().clone();
            let comp_mut = temp_proj.active_composition_mut();
            let m_count = comp_mut.markers.len() + 1;
            comp_mut.markers.push(crate::core::timeline::TimelineMarker {
                frame: current_frame,
            ...
            app.toasts.info(format!("Added marker at frame {}", current_frame));
        }
    });
                ...
                for (m_idx, marker) in markers_list.iter().enumerate() {
                    ...
                    ui.horizontal(|ui| {
                        if ui.small_button("⏩ Jump").on_hover_text("Jump Playhead to this marker frame").clicked() {
                            app.current_frame = marker.frame;
                        }
