...
pub fn draw(app: &mut KagariApp, ctx: &egui::Context) {
    ...
    egui::Window::new("✨ Welcome")
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                if ui
                    .add(egui::Button::new("🎬 Load Demo Scene").min_size(egui::vec2(150.0, 34.0)))
                    .on_hover_text("Instant animated composition — press Space to play")
                    .clicked()
                {
                ...
                if ui
                    .add(egui::Button::new("＋ New Comp (Cmd+N)").min_size(egui::vec2(160.0, 34.0)))
                    .clicked()
                {
                    app.show_welcome = false;
            ...
            if !recents.is_empty() {
                ...
                for rp in recents.iter().take(5) {
                    ...
                    ui.horizontal(|ui| {
                        if ui
                            .small_button(egui::RichText::new(format!("📄 {}", fname)).small())
                            .on_hover_text(rp)
                            .clicked()
                        {
            ...
            ui.separator();
            ui.label(
                egui::RichText::new("🡇 Drop PNG / JPG / MP4 / WAV files anywhere to import")
                    .color(colors::TEXT_ACCENT),
            );
            ui.add_space(6.0);
            egui::Grid::new("welcome_keys")
                .num_columns(2)
                .spacing([12.0, 3.0])
                .show(ui, |ui| {
                    for (k, d) in [
