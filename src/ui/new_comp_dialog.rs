    ...
    egui::Window::new("🆕 New Composition")
        ...
        .show(ctx, |ui| {
            ...
            let mut draft = ctx.data_mut(|d| {
                d.get_temp::<(String, usize, u32, u32, u32, u32)>(id).unwrap_or_else(|| {
                    (format!("Composition {}", count + 1), 0usize, 1920, 1080, 30, 300)
                })
            });
            ...
            ui.add_space(6.0);
            ui.label(egui::RichText::new("Preset").small().color(colors::TEXT_SECONDARY));
            let mut picked = None;
            ...
            ui.add_space(6.0);
            egui::Grid::new("newcomp_fields").num_columns(2).spacing([10.0, 4.0]).show(ui, |ui| {
                ui.label("Width:");
                ui.add(egui::DragValue::new(&mut draft.2).range(16..=7680).suffix(" px"));
                ui.label("Height:");
                ui.add(egui::DragValue::new(&mut draft.3).range(16..=4320).suffix(" px"));
                ui.end_row();
                ui.label("Frame Rate:");
                ui.add(egui::DragValue::new(&mut draft.4).range(1..=120).suffix(" fps"));
                ui.label("Duration:");
                ui.add(egui::DragValue::new(&mut draft.5).range(1..=180_000).suffix(" fr"));
                ui.end_row();
            });
            if draft.1 + 1 < PRESETS.len() {
                ui.label(egui::RichText::new("Editing fields switches to Custom").small().color(colors::TEXT_MUTED));
            }

            ui.add_space(8.0);
            ui.separator();
            ui.horizontal(|ui| {
                if ui.add(egui::Button::new("✓ Create").min_size(egui::vec2(90.0, 26.0))).clicked() {
                    let name = if draft.0.trim().is_empty() { format!("Composition {}", count + 1) } else { draft.0.trim().to_string() };
