...
use aftereffects_oss::core::audio_spectrum::{
    extract_waveform, render_spectrum, AudioSpectrumOptions, AudioSpectrumType,
    SpectrumAnalyzer,
};
...
fn test_full_distortion_and_grade_pipeline_is_deterministic() {
    let run = || {
        ...
        // Distortion stage.
        apply_wave_warp_pro(&mut img, 48, 48, &WaveWarpParams {
            wave_height: 6.0, wave_width: 16.0, speed: 1.0, time: 0.75,
            wave_type: WaveType::Sine, pinning: PinKind::All, ..Default::default()
        });
        apply_cc_lens_pro(&mut img, 48, 48, &CcLensParams { convergence: 35.0, zoom: 1.0 });
        apply_polar_coordinates_pro(&mut img, 48, 48, PolarMode::RectToPolar, 0.5);
        apply_optics_compensation(&mut img, 48, 48, &OpticsCompensationParams {
            field_of_view_deg: 45.0, reverse: false, zoom: 1.0,
        });
        apply_cc_bend_it_pro(&mut img, 48, 48, 2.0, -2.0);
        // Stylize stage.
        apply_light_sweep(&mut img, 48, 48, &LightSweepParams {
            direction_deg: 30.0, center: 0.4, width: 0.3,
            sweep_intensity: 0.5, edge_intensity: 0.2,
        });
        apply_glow_pro(&mut img, 48, 48, 0.55, 3, 0.8);
        // Grade stage.
        let s_curve = ToneCurve::new(vec![[0.0, 0.0], [0.25, 0.18], [0.5, 0.5], [0.75, 0.82], [1.0, 1.0]]);
        apply_curves(&mut img, &ChannelCurves { master: Some(s_curve), ..Default::default() });
        apply_color_balance(&mut img, &ColorBalance {
            shadows: [-15.0, 5.0, 20.0],
            midtones: [10.0, 0.0, -10.0],
            highlights: [25.0, -5.0, -15.0],
            preserve_luminosity: true,
        });
        apply_channel_mixer(&mut img, &ChannelMixer {
...
fn test_registry_parameter_sweep_never_panics() {
    let variants = vec![
        ExtEffect::WaveWarp { wave_height: 40.0, wave_width: 8.0, speed: 3.0, direction_deg: 270.0, wave_type: 3, pinning: 3 },
        ExtEffect::CcLens { convergence: -100.0, zoom: 3.0 },
        ExtEffect::PolarCoordinates { to_polar: false, interpolation: 100.0 },
        ExtEffect::OpticsCompensation { field_of_view_deg: -179.0, reverse: true, zoom: 0.05 },
        ExtEffect::Curves { master: vec![[0.0, 1.0], [1.0, 0.0]], red: vec![], green: vec![], blue: vec![] },
        ExtEffect::ColorBalance { shadows: [100.0; 3], midtones: [-100.0; 3], highlights: [100.0; 3], preserve_luminosity: false },
        ExtEffect::ChannelMixer { matrix: [[300.0; 3]; 3], monochrome: true },
        ExtEffect::LightSweep { direction_deg: 180.0, center: 0.0, width: 1.0, sweep_intensity: 1.0, edge_intensity: 1.0 },
        ExtEffect::RadialFastBlur { amount: 1.0, samples: 64 },
        ExtEffect::BendIt { top_offset: 50.0, bottom_offset: -50.0 },
        ExtEffect::Tiler { scale_percent: 5000.0, mirror: false },
        ExtEffect::GlowPro { threshold: 0.0, radius: 128, intensity: 4.0 },
    ];
    for e in &variants {
...
fn test_particle_simulation_with_all_forces_renders() {
    ...
    for p in &ps.particles {
        assert!(p.x.is_finite() && p.y.is_finite(), "positions must stay finite");
        assert!(p.x >= -1.0 && p.x <= 321.0, "collision must contain x: {}", p.x);
        assert!(p.y >= -1.0 && p.y <= 241.0, "collision must contain y: {}", p.y);
    }
    let mut buf = vec![0u8; 320 * 240 * 4];
    ps.render(&mut buf, 320, 240, 1.0);
    assert!(buf.chunks(4).any(|px| px[3] > 0), "rendered frame must contain particles");
}
...
fn test_audio_pipeline_end_to_end() {
    ...
    render_spectrum(&mut buf, 256, 128, &bands, &opts, [10, 30, 80], [120, 230, 255]);
    assert!(buf.chunks(4).any(|px| px[3] == 255), "spectrum must draw bars");

    let wf = extract_waveform(&pcm, 16);
    assert_eq!(wf.len(), 16);
    assert!(wf.iter().any(|&v| v > 0.4), "waveform envelope must capture the tone");
}
...
fn test_text_animator_stack_with_color_channels() {
    ...
    let mut tracking_anim = TextAnimatorAdvanced {
        selector: RangeSelector { shape: SelectorShape::Square, start: 0.0, end: 100.0, ..Default::default() },
        tracking: 8.0,
        ..Default::default()
    };
    ...
    let stack = AnimatorStack { animators: vec![pop, tracking_anim] };
