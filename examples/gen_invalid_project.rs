...
fn main() {
    ...
    let mut bg = Layer::new("bg".into(), "BG".into(), LayerType::Solid { color: [0.2; 4] }, 30);
    ...
    let back = Layer::new("pcb".into(), "BackToRoot".into(), LayerType::PreComp { comp_id: "comp1".into() }, 30);
    sub_b.layers.push(back);
    comp.sub_compositions.push(sub_b);

    // PreComp layer referencing B
    let mut pc = Layer::new("pc1".into(), "Nested".into(), LayerType::PreComp { comp_id: "B".into() }, 30);
    pc.transform.position = Animatable::new_constant([32.0, 32.0]);
    comp.layers.push(pc);

    // Missing pre-comp reference
    let missing = Layer::new("miss".into(), "MissingRef".into(), LayerType::PreComp { comp_id: "GHOST".into() }, 30);
    comp.layers.push(missing);

    // Suspicious scale
    let mut bad = Layer::new("bad".into(), "BadScale".into(), LayerType::Solid { color: [1.0; 4] }, 30);
